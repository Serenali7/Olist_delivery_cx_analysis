...
-- 03-04-01 Baseline View
CREATE OR REPLACE VIEW v_baseline AS
SELECT
  AVG(seller_prep_time_days) AS baseline_seller_days,
  AVG(carrier_lead_time_days) AS baseline_carrier_days
FROM v_order_metrics_clean
WHERE is_late = 0;

-- 03-04-02 Excess View
CREATE OR REPLACE VIEW v_delay_decomp_excess AS
SELECT
  m.order_id,
  m.is_late,
  m.delay_days,
  m.seller_prep_time_days,
  m.carrier_lead_time_days,
  GREATEST(
    m.seller_prep_time_days - b.baseline_seller_days,
    0
  ) AS seller_excess_days,
  GREATEST(
    m.carrier_lead_time_days - b.baseline_carrier_days,
    0
  ) AS carrier_excess_days
FROM v_order_metrics_clean m
CROSS JOIN v_baseline b
WHERE m.is_late = 1;

-- 03-04-03 Attribution
SELECT
  CASE
    WHEN carrier_excess_days > seller_excess_days THEN 'carrier_driven'
    WHEN seller_excess_days > carrier_excess_days THEN 'seller_driven'
    ELSE 'mixed'
  END AS delay_driver,
  COUNT(*) AS orders,
  ROUND(COUNT(*) * 1.0 / SUM(COUNT(*)) OVER (), 4) AS pct_orders
FROM v_delay_decomp_excess
...
CREATE OR REPLACE VIEW v_seller_order_level AS
SELECT DISTINCT
  oi.seller_id,
  om.order_id,
  om.is_late
